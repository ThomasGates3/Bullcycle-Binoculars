...
export async function analyzeSentiment(
  ...
  try {
    ...
    logger.warn('Bedrock sentiment analysis failed, trying fallback', { error: error instanceof Error ? error.message : error });

    if (sagemakerEndpoint) {
      try {
        return await analyzeSentimentSageMaker(title, snippet, sagemakerEndpoint, region);
      } catch (smError) {
        logger.warn('SageMaker sentiment analysis failed, using keyword fallback', { error: smError instanceof Error ? smError.message : smError });
      }
    }
...
async function analyzeSentimentBedrock(title: string, snippet: string, modelId: string, region: string): Promise<SentimentResult> {
...
async function analyzeSentimentSageMaker(title: string, snippet: string, endpoint: string, region: string): Promise<SentimentResult> {
...
function analyzeSentimentKeyword(title: string, snippet: string): SentimentResult {
...
function mapSentiment(label: string, score: number): SentimentResult {
  let sentiment: 'Positive' | 'Negative' | 'Neutral';
